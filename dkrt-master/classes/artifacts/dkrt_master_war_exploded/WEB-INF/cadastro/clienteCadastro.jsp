...
<span class="geral">
	 <form method="post" action="controller">
         <c:choose>
             ...
             <c:otherwise>
                 <input type="hidden" value="cadastro" name="acao">
                 <input type="hidden" value="editarCliente" name="tipo">
				...
				<label>RG *: <input type="text" id="rgCliente" name="rgCliente" class="form-control"
                                    value="${clienteBusca.rg}" required/></label>
				<label>CPF *: <input type="text" onclick="limpaCampo()" onblur="mostraErro(this.value)" id="cpfCliente"
                                     maxlength="11" name="cpfCliente" onkeyup="validarCpf(this.value)"
                                     class="form-control" value="${clienteBusca.cpf}" required/></label>
				<label>Dt. Nascimento *: <input type="text" onblur="validaData()" id="dtNascCliente"
                                                name="dtNascCliente" class="form-control"
                                                value="<fm:formatDate value="${clienteBusca.dtNascimento}" pattern="dd/MM/yyyy"/>"
                                                required/></label>
				<label>E-mail *: <input type="text" onclick="limpaCampo()" onblur="mostraErro(this.value)"
                                        id="emailCliente" onkeyup="validarEmail(this.value)" name="emailCliente"
                                        class="form-control" value="${clienteBusca.email}" required/></label>
				<label>Celular *: <input type="text" name="celularCliente" class="form-control"
                                         value="${clienteBusca.celular}" required/></label> <br>
				<label>Telefone: <input type="text" name="telefoneCliente" class="form-control"
                                        value="${clienteBusca.telefone}"/></label> <br>
         <label>CEP *: <input type="text" name="cepEnderecoCliente"
                              class="form-control" onblur="pesquisacep(this.value)"
                              value="${clienteBusca.enderecoDTO.cep}" id="cep" required/></label>
				<label>Rua *: <input type="text" name="ruaEnderecoCliente" id="rua" class="form-control"
                                     required/></label>
				<label>Numero *: <input type="text" name="numeroEnderecoCliente"
         ...
         <select name="clienteUfId" class="form-control" style="width: 300px;">
            ...
            <c:forEach var="uf" items="${listaUfs}">
                <c:if test="${uf.sigla != 'N/A' && uf.sigla != clienteBusca.enderecoDTO.ufDTO.sigla}">
